...
def print_result(result: dict, verbose: bool = False):
    ...
    if not result.get("success", False):
        print(f"\n[ERROR] 操作失败: {result.get('error', 'Unknown error')}")
        return
...
def print_space_analysis(data: dict):
    ...
    if summary.get("critical_drives"):
        print(f"[WARNING]  危险驱动器: {', '.join(summary['critical_drives'])}")
    if summary.get("warning_drives"):
        print(f"[WARNING]  警告驱动器: {', '.join(summary['warning_drives'])}")
...
def print_recommendations(data: dict):
    ...
    print(f"\n[IDEA] 清理建议 ({len(recommendations)} 条):")
    for rec in recommendations:
        priority_icon = {"high": "🔴", "medium": "🟡", "low": "🟢"}.get(rec.get("priority", "low"), "⚪")
