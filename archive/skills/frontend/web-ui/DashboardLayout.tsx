...
```tsx
// 使用示例：App.tsx 或 page.tsx
import DashboardLayout, { DashboardModule } from './DashboardLayout';

const App = () => {
  return (
    <DashboardLayout userRole="analyst">
      <DashboardModule colSpan={4} title="实时数据监控">
        {/* 图表组件 */}
        <div className="h-48 bg-gradient-to-b from-blue-50 to-white rounded-lg" />
      </DashboardModule>
      
      <DashboardModule colSpan={4} title="算法透明度">
        <div className="space-y-3">
          <div className="flex justify-between">
            <span className="text-sm">决策可信度</span>
      ...
      <DashboardModule colSpan={4} title="协同工作区">
        <div className="text-center py-8 text-gray-500">
          拖拽模块至此开始协作
        </div>
      </DashboardModule>
      
      <DashboardModule colSpan={8} rowSpan={2} title="数据溯源图谱">
        <div className="h-64 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg 
          border-2 border-dashed border-gray-300" />
      </DashboardModule>
      
      <DashboardModule colSpan={4} title="离线推演">
...
这个组件完全遵循设计系统规范，实现了智能情境感知、多通道交互和伦理透明化等核心原则，同时保持了代码的模块化和可维护性。