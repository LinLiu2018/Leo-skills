...
try:
    ...
    print("[ERROR] 请先安装 Streamlit: pip install streamlit")
    print("   或使用: pip install -e .[web]")
    sys.exit(1)

# 页面配置
st.set_page_config(
    page_title="Leo AI System", page_icon="🤖", layout="wide", initial_sidebar_state="expanded"
)
...
def load_system():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "leo_system", str(PROJECT_ROOT / "leo-system.py")
        )
...
def render_metrics(system, stats):
    ...
    with col1:
        ...
        st.metric(label="[TARGET] Skills", value=skills_count, delta="已加载")

    with col2:
        agents_count = stats["agents"]["total"] if stats else 0
        st.metric(label="🤖 Agents", value=agents_count, delta="已创建")

    with col3:
        exec_count = stats["statistics"]["total_executions"] if stats else 0
        st.metric(label="📈 执行次数", value=exec_count)

    with col4:
        success_rate = stats["statistics"].get("success_rate", 0) if stats else 0
        st.metric(label="[SUCCESS] 成功率", value=f"{success_rate:.1f}%")


def render_skills_panel(system, stats):
    """渲染 Skills 面板"""
    st.subheader("[TARGET] Skills 技能库")
        ...
        if agent_names:
            for agent_name in agent_names:
                with st.container():
                    ...
                    with col3:
                        st.markdown(
                            '<span class="status-active">🟢 就绪</span>', unsafe_allow_html=True
                        )
        else:
            st.info("暂无 Agent")
    else:
...
def render_task_executor(system):
    """渲染任务执行器"""
    st.subheader("[LAUNCH] 任务执行器")

    with st.form("task_form"):
        task_input = st.text_area(
            "输入任务描述", placeholder="例如：生成营销文档、分析数据、创作内容...", height=100
        )
        ...
        with col2:
            submitted = st.form_submit_button("[LAUNCH] 执行任务", type="primary")
            ...
            with st.spinner("执行中..."):
                try:
                    ...
                    if result.get("success", True):
                        st.success("[SUCCESS] 任务执行成功")
                        st.json(result)
                    else:
                        st.error(f"[ERROR] 执行失败: {result.get('error', '未知错误')}")
                except Exception as e:
                    st.error(f"[ERROR] 执行出错: {str(e)}")
...
def main():
    ...
    if error:
        st.error(f"[WARNING] 系统加载失败: {error}")
