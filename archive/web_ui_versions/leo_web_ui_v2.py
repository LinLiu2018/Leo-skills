...
def render_header():
    ...
    st.markdown(
        '<p class="sub-header">Skills + Subagents + Workflows 协同工作架构 | v2.0</p>',
        unsafe_allow_html=True,
    )
        ...
        for tab, category in zip(tabs, categories):
            with tab:
                ...
                if len(skills) > page_size:
                    page = (
                        st.selectbox(f"页码", range(1, total_pages + 1), key=f"page_{category}") - 1
                    )
                else:
                    page = 0
...
def render_sidebar(loader, agents):
    ...
    with st.sidebar:
        ...
        if st.button("[DATA] 导出报告", use_container_width=True):
            if loader:
...
def main():
    ...
    # 错误提示
    if loader_error:
        st.error(f"[WARNING] Skills 加载失败: {loader_error}")
    if agents_error:
        st.warning(f"[WARNING] Agents 加载警告: {agents_error}")
