...
st.set_page_config(
    ...
    menu_items={"About": "Leo AI Agent System v3.0 - 您的智能工作伙伴"},
)
...
def render_header():
    """渲染页头"""
    st.markdown(
        '<h1 class="hero-title animate-in">[LAUNCH] Leo AI 智能工作台</h1>', unsafe_allow_html=True
    )
    st.markdown(
        f"""
...
def render_metrics(data):
    ...
    with col1:
        ...
        <div class="metric-card">
            ...
            <div class="metric-label">[TARGET] Skills 技能</div>
        </div>
        """,
            unsafe_allow_html=True,
        )
...
def render_selected_scenario():
    ...
    if st.button("[LAUNCH] 使用此配置执行任务", type="primary"):
    ...
    for tab, cat in zip(tabs, sorted_cats):
        with tab:
            ...
            search = st.text_input(
                "[SEARCH] 搜索", key=f"search_{cat}", placeholder="输入关键词..."
            )
    ...
    for name, meta in agents.items():
        with st.container():
            ...
            with col1:
                st.markdown(f"**🤖 {name}**")
                st.caption(
                    f"类型: {meta.type} | {meta.description[:50] if meta.description else '智能代理'}..."
                )
            with col2:
                st.markdown("🟢 就绪")
...
def render_task_executor(data):
    """渲染任务执行器"""
    st.markdown("### [LAUNCH] 任务执行器")

    st.markdown('<div class="executor-box">', unsafe_allow_html=True)

    with st.form("task_form"):
        task = st.text_area(
            "[NOTE] 任务描述", placeholder="例如：为建华观园项目生成营销文档...", height=100
        )
        ...
        submitted = st.form_submit_button(
            "[LAUNCH] 执行任务", type="primary", use_container_width=True
        )

        if submitted and task:
            with st.spinner("执行中..."):
                st.success("[SUCCESS] 任务已提交！")
                st.json(
                    {
                        "task": task[:100],
                        "agent": agent,
...
def render_sidebar(data):
    """渲染侧边栏"""
    with st.sidebar:
        st.markdown("## [LAUNCH] Leo AI System")
        ...
        if st.button("[DATA] 系统状态", use_container_width=True):
...
def main():
    ...
    <div style="text-align: center; color: #64748b; padding: 1rem;">
        [LAUNCH] Leo AI Agent System v3.0 | Built with ❤️ by Leo Liu | Powered by Streamlit
    </div>
    """,
        unsafe_allow_html=True,
    )
