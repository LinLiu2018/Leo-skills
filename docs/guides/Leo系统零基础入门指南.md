...
│  💾 职责: 技能(107个)、代理(9个)、工作流(5条)               │
│  ⚙️ 存储: 本地 src/ 目录                                     │
└──────────┬──────────────────────────────┬───────────────────┘
           │                              │
           ▼                              ▼
┌──────────────────────────┐  ┌──────────────────────────────┐
│  OpenClaw (大龙虾/小龙虾) │  │  Superpowers (v4.2.0)        │
│  📁 位置: D:\moltbot       │  │  📁 位置: ~/.claude/skills/   │
│  💾 职责: 消息网关、飞书   │  │  💾 职责: 开发工作流技能      │
│  ☁️ 部署: 本地 + 云端      │  │  🔧 14个: TDD/调试/审查/计划  │
└──────────┬───────────────┘  └──────────────────────────────┘
           │
           ▼
┌─────────────────────────────────────────────────────────────┐
...
- **Superpowers** = 电脑的**专业开发工具包**（TDD、调试、代码审查等）
- **飞书** = 电脑的**显示器和键盘**（你看的和操作的界面）

**两条数据流**：
```
路径1 (飞书用户):
你说的话 (飞书) → 大龙虾 Gateway → Leo AI System → 大龙虾 → 飞书 (你看到回答)

路径2 (开发者):
Claude Code (VSCode) → Superpowers Hook → 开发工作流技能 → 开发输出
```
...
| Superpowers | `~/.claude/skills/superpowers/` | 14个开发工作流技能 |
...
| 组件类型 | 数量 | 成熟度 | 建议 |
|---------|------|--------|------|
| Skills | 107 | ⭐⭐⭐⭐ | 核心技能成熟，集成 obra/superpowers 14个专业开发技能 |
| Agents | 9 | ⭐⭐⭐⭐ | 房产/研究/创作最成熟 |
| Workflows | 5 | ⭐⭐⭐⭐ | 设计完善，待实战验证 |
...
| Superpowers | `~/.claude/skills/superpowers/` | `skills/`, `hooks/`, `agents/` |
| 飞书配置 | `C:\Users\刘方林` | `.openclaw/openclaw.json` |

---

*本文档最后更新：2026-02-06 | 说"更新入门指南"可刷新*
