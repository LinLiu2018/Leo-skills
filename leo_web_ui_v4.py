...
try:
    ...
    print("❌ 请先安装依赖: pip install streamlit streamlit-autorefresh")
    sys.exit(1)
...
def render_task_monitor():
    ...
    if not tasks:
        ...
        for task in sorted_tasks:
            ...
            with st.expander(f"[{task['status'].upper()}] {task['description'][:50]}...", expanded=(task['status'] == 'running')):
...
def render_execution_panel():
    ...
    with st.form("execution_form"):
        ...
        with col2:
            workflows = ["不使用 Workflow"] + [f"⚡ {name}" for name in core["loader"].workflows.keys()]
            workflow = st.selectbox("选择 Workflow", workflows)
        with col3:
            projects = ["无关联项目"] + [p['name'] for p in core.get("projects", [])] if "projects" in core else ["无关联项目"]
