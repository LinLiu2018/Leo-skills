...
class DeAIifier:
    ...
    def process(self, text: str, mode: Optional[Literal["creative", "formal"]] = None) -> str:
        """
        处理文本，应用去AI化规则
    ...
    def _replace_marketing_jargon(self, text: str) -> str:
        """替换营销黑话为更自然的表达"""
        jargon_replacements = self.creative_config.get("marketing_jargon_replacements", {})
    ...
    def check_quality(
        self, text: str, mode: Optional[Literal["creative", "formal"]] = None
    ) -> Dict[str, any]:
        """
        检查内容质量
...
def deaiify(
    text: str, mode: Literal["creative", "formal"] = "creative", config_path: Optional[str] = None
) -> str:
    """
    去AI化处理文本
