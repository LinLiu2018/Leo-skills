...
lead_fields = [
    ...
    {"name": "parent_id", "type": "integer", "foreign_key": "leads.id", "description": "推荐人ID"},
    {
        ...
        "default": "new",
        "max_length": 20,
        "description": "状态(new/contacted/converted)",
    },
...
reward_fields = [
    ...
    },
    {
        ...
        "required": True,
        "max_length": 20,
        "description": "奖励类型(coupon/cash/points)",
    },
    {"name": "amount", "type": "float", "required": True, "description": "奖励金额"},
    {
        ...
        "default": "pending",
        "max_length": 20,
        "description": "状态(pending/issued/used)",
    },
    {"name": "issued_at", "type": "datetime", "description": "发放时间"},
]

reward_results = db_gen.generate(
    entity_name="Reward", fields=reward_fields, indexes=["lead_id", "status"]
)
...
lead_api_results = api_gen.generate(
    resource_name="lead", fields=lead_api_fields, auth_required=False  # 小程序端不需要JWT
)
...
reward_api_results = api_gen.generate(
    resource_name="reward", fields=reward_api_fields, auth_required=False
)
...
share_results = mp_gen.generate(
    page_name="share", page_type="share", data_bindings=[], features=["share", "poster"]
)
