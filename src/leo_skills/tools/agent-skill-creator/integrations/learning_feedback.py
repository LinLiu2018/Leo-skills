...
class LearningFeedbackSystem:
    ...
    def analyze_agent_usage(
        ...
        try:
            ...
            milestone = self._check_for_milestone(
                agent_name, execution_time, success, result_quality
            )
            if milestone:
    ...
    def _check_for_milestone(
        self, agent_name: str, execution_time: float, success: bool, result_quality: float
    ) -> Optional[LearningMilestone]:
    ...
    def _detect_improvement(
        self, agent_name: str, execution_time: float, result_quality: float
        ...
        if result_quality > 0.9:
            return {
                "type": "quality",
                "message": f"[SPARKLE] High quality results detected",
    ...
    def _generate_pattern_feedback(self, agent_name: str, user_input: str) -> Optional[str]:
        ...
        if (
            6 <= hour <= 9
            and len([q for q in queries[-5:] if "morning" in q.lower() or "today" in q.lower()])
            >= 3
        ):
        ...
        if (
            weekday == 4
            and len([q for q in queries[-10:] if "week" in q.lower() or "friday" in q.lower()]) >= 2
        ):
            return f"[DATA] {agent_name} is preparing your weekly summary"
    ...
    def _format_milestone_feedback(self, milestone: LearningMilestone) -> str:
        ...
        messages = {
            ...
            "consistency": [
                f"[TARGET] Excellent! {milestone.description}",
                f"[TARGET] Your agent has proven its reliability",
            ],
            "speed_improvement": [
        ...
        message_set = messages.get(milestone.milestone_type, ["[SPARKLE] Milestone achieved!"])
        return message_set[0] if message_set else f"[SPARKLE] {milestone.description}"
    ...
    def get_learning_summary(self, agent_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = (
                (sum(pattern.get("successes", [])) / len(pattern.get("successes", [False])) * 100)
                if pattern.get("successes")
                else 0
            )
            avg_time = (
                sum(pattern.get("execution_times", [])) / len(pattern.get("execution_times", [1]))
                if pattern.get("execution_times")
                else 0
            )
            ...
            milestones = [
                m
                for m in self.milestones_achieved
                if m.description and agent_name.lower() in m.description.lower()
            ]
        ...
        try:
            ...
            if themes:
                ...
                if themes[top_theme] >= 3:  # Appears in 3+ recent queries
                    return f"[TARGET] I notice you often ask about {top_theme}. Consider creating a specialized agent for this."
...
def analyze_agent_execution(
    agent_name: str, user_input: str, execution_time: float, success: bool, result_quality: float
) -> Optional[str]:
    """
    ...
    return system.analyze_agent_usage(
        agent_name, user_input, execution_time, success, result_quality
    )
