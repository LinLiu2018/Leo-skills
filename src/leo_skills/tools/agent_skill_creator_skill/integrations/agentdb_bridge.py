...
class AgentDBBridge:
    ...
    def _check_cli_availability(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["agentdb", "--help"], capture_output=True, text=True, timeout=10
            )
    ...
    def _attempt_automatic_install(self) -> bool:
        ...
        try:
            ...
            installation_methods = [self._install_npm_global, self._install_npx_fallback]
    ...
    def _auto_configure(self):
        ...
        try:
            ...
            config = {
                "reflexion": {"auto_save": True, "compression": True},
                "causal": {"auto_track": True, "utility_model": "outcome_based"},
                "skills": {"auto_extract": True, "success_threshold": 0.8},
                "nightly_learner": {"enabled": True, "schedule": "2:00 AM"},
            }
    ...
    def _enhance_with_real_agentdb(
        ...
        try:
            ...
            skills_result = self._execute_agentdb_command(
                [
                    "agentdb" if self.use_cli else "npx",
                    "agentdb",
                    "skill",
                    "search",
                    user_input,
            ...
            if skills_result:
                ...
                if skills:
                    intelligence.learned_improvements = [
                        f"Skill available: {skill.get('name', 'unknown')}" for skill in skills[:3]
                    ]
            ...
            if episodes_result:
                ...
                if episodes:
                    success_rate = sum(1 for e in episodes if e.get("success", False)) / len(
                        episodes
                    )
            ...
            if domain:
                ...
                if causal_result:
                    ...
                    if effects:
                        ...
                        intelligence.mathematical_proof = (
                            f"Causal uplift: {best_effect.get('uplift', 0):.2%}"
                        )
    ...
    def _enhance_with_legacy_agentdb(
        self, user_input: str, domain: str = None
    ) -> AgentDBIntelligence:
        ...
        for line in lines:
            if "→" in line and "uplift" in line.lower():
                ...
                if len(parts) >= 2:
                    ...
                    effects.append({"cause": cause, "effect": effect, "uplift": uplift})
    ...
    def _execute_agentdb_command(self, command: List[str]) -> Optional[str]:
        ...
        try:
            result = subprocess.run(
                command, capture_output=True, text=True, timeout=30, cwd=str(Path.cwd())
            )
    ...
    def enhance_template(self, template_name: str, domain: str) -> Dict[str, Any]:
        ...
        try:
            ...
            success_result = self._execute_agentdb_command(
                ["npx", "agentdb", "causal", "recall", f"template_success_rate:{template_name}"]
            )

            if success_result:
                try:
                    success_data = json.loads(success_result)
                    enhancements["agentdb_integration"]["success_rate"] = success_data.get(
                        "success_rate", 0.8
                    )
                    enhancements["agentdb_integration"]["historical_usage"] = success_data.get(
                        "usage_count", 0
                    )
                except:
            ...
            # Get learned improvements
            improvements_result = self._execute_agentdb_command(
                ["npx", "agentdb", "skills", "list", f"template:{template_name}"]
            )

            if improvements_result:
                enhancements["agentdb_integration"]["learned_improvements"] = (
                    self._parse_improvements(improvements_result)
                )
    ...
    def get_learning_summary(self, agent_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            reflexion_result = self._execute_agentdb_command(
                ["npx", "agentdb", "reflexion", "recall", f"agent:{agent_name}", "--format", "json"]
            )
            ...
            # Get learned skills
            skills_result = self._execute_agentdb_command(
                ["npx", "agentdb", "skills", "list", f"agent:{agent_name}"]
            )
            ...
            # Get causal patterns
            causal_result = self._execute_agentdb_command(
                ["npx", "agentdb", "causal", "recall", f"agent:{agent_name}", "--format", "json"]
            )
