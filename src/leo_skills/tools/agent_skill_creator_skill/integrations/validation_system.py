...
class MathematicalValidationSystem:
    ...
    def validate_template_selection(
        self, template: str, user_input: str, domain: str
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            logger.info(
                f"Template validation: {template} - {confidence:.1%} confidence - {'✓' if is_valid else '✗'}"
            )
    ...
    def validate_architecture(
        self, structure: Dict, complexity: str, domain: str
    ) -> ValidationResult:
        """
    ...
    def _calculate_template_confidence(
        self, template: str, historical_data: Dict, user_input: str
    ) -> float:
        ...
        min(0.2, historical_data.get("usage_count", 0) / 1000)
