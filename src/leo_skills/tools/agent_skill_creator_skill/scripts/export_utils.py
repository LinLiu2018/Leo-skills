                    ...
                    for line in frontmatter.split("\n"):
                        ...
                        if line.startswith("name:"):
                            ...
                            if name_length > MAX_NAME_LENGTH:
                                issues.append(
                                    f"name too long: {name_length} chars (max {MAX_NAME_LENGTH})"
                                )
                            ...
                            if desc_length > MAX_DESCRIPTION_LENGTH:
                                issues.append(
                                    f"description too long: {desc_length} chars (max {MAX_DESCRIPTION_LENGTH})"
                                )
...
[SUCCESS] Optimized for Claude Desktop and claude.ai manual upload
...
[SUCCESS] Optimized for programmatic Claude API integration
...
[SUCCESS] **Your skill is now available in Claude Desktop!**
...
[SUCCESS] **Your skill is now available at claude.ai!**
...
[SUCCESS] **Your skill is now integrated with the Claude API!**
...
| **Version Control** | [SUCCESS] Native | [WARNING] Manual | [SUCCESS] Versioned |
| **Team Sharing** | [SUCCESS] Via plugins | [ERROR] Individual | [SUCCESS] Via API |
| **marketplace.json** | [SUCCESS] Used | [ERROR] Ignored | [ERROR] Not used |
...
**Generated by:** agent_skill_creator_skill v3.2 cross-platform export system
"""
...
def export_skill(
    ...
    print("[SEARCH] Validating skill structure...")
    valid, issues = validate_skill_structure(skill_path)
    if not valid:
        return {"success": False, "message": "Skill validation failed", "issues": issues}
    print("[SUCCESS] Skill structure valid")
    ...
    # Create packages
    results = {"success": True, "version": version, "packages": {}}
    ...
    if "desktop" in variants:
        ...
        if desktop_result["success"]:
            print(
                f"[SUCCESS] Desktop package: {os.path.basename(desktop_result['zip_path'])} ({desktop_result['size_mb']:.2f} MB)"
            )
        else:
            print(f"[ERROR] Desktop package failed: {desktop_result['message']}")
            results["success"] = False

    if "api" in variants:
        print(f"\n🔨 Creating API package...")
        api_result = create_export_package(skill_path, output_dir, "api", version, skill_name)
        results["packages"]["api"] = api_result
        if api_result["success"]:
            print(
                f"[SUCCESS] API package: {os.path.basename(api_result['zip_path'])} ({api_result['size_mb']:.2f} MB)"
            )
        else:
            print(f"[ERROR] API package failed: {api_result['message']}")
            results["success"] = False
    ...
    if results["success"]:
        ...
        print(f"[SUCCESS] Installation guide: {os.path.basename(guide_path)}")
...
"""
    ...
    # Run export
    print(f"\n[LAUNCH] Exporting skill: {os.path.basename(skill_path)}\n")
    ...
    if results["success"]:
        ...
        print(f"\n[TARGET] All files saved to: {output_dir or 'exports/'}")
    else:
