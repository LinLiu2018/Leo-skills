...
class ContentAnalyzer:
    ...
    def analyze(self, content: Any) -> AnalysisResult:
        ...
        logger.info(
            f"Analysis complete: domain={domain}, complexity={complexity}, confidence={confidence:.2f}"
        )
    ...
    def detect_algorithms(self, content: Any) -> List[Algorithm]:
        ...
        for match in self.algorithm_pattern.finditer(text):
            ...
            complexity = None
            complexity_match = self.complexity_pattern.search(
                text[match.start() : match.end() + 500]
            )
            if complexity_match:
        ...
        for code_block in content.code_blocks:
            if self._is_algorithmic_code(code_block.code):
                algorithms.append(
                    Algorithm(
                        name=(
                            code_block.context[:50] if code_block.context else "Detected Algorithm"
                        ),
                ...
                for match in matches:
                    ...
                    if lib_name and len(lib_name) > 1:
                        dependencies[lib_name] = Dependency(
                            name=lib_name, version=None, purpose="Detected from imports"
                        )
            ...
            for dep in content.metadata["dependencies"]:
                if dep not in dependencies:
                    dependencies[dep] = Dependency(
                        name=dep, version=None, purpose="Detected from notebook"
                    )
    ...
    def _calculate_confidence(
        self, algorithms: List[Algorithm], architectures: List[Architecture], domain: str
    ) -> float:
