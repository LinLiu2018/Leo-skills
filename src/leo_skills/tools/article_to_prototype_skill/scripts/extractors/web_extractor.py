...
from .pdf_extractor import CodeBlock, ExtractedContent, Section
...
class WebExtractor:
    ...
    def __init__(self, timeout: int = 30, max_retries: int = 3, user_agent: Optional[str] = None):
        ...
        self.user_agent = user_agent or ("Mozilla/5.0 (compatible; Article-to-Prototype/1.0)")
    ...
    def _extract_code_blocks_bs4(self, content: BeautifulSoup) -> List[CodeBlock]:
        ...
        for i, code_element in enumerate(content.find_all(["pre", "code"])):
            ...
            code_blocks.append(
                CodeBlock(language=language, code=code_text, line_number=i, context=context)
            )
    ...
    def _parse_text_into_sections(self, text: str) -> List[Section]:
        ...
        for i, line in enumerate(lines):
            ...
            if match and len(stripped) > 3 and len(stripped) < 100:
                ...
                current_section = Section(
                    heading=heading, level=level, content="", line_number=i, subsections=[]
                )
                current_content = []
            elif current_section:
                if stripped:
    ...
    def crawl_documentation(
        self, base_url: str, max_pages: int = 10, follow_pattern: Optional[str] = None
