...
class ArticleToPrototype:
    ...
    def __init__(self):
        ...
        super().__init__(
            skill_name="article_to_prototype_skill",
    ...
    def process(
        self, source: str, output_dir: str, language_hint: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            language = self.language_selector.select_language(analysis, hint=language_hint)
            ...
            result = self.prototype_generator.generate(analysis, language, output_dir, source_info)

            logger.info(f"[SUCCESS] Successfully generated prototype in: {output_dir}")
            ...
            logger.error(f"[ERROR] Failed to process article: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
...
def main():
    ...
    parser.add_argument("source", help="Path to PDF, URL, notebook, or markdown file")
    parser.add_argument(
        "-o", "--output", default="./output", help="Output directory (default: ./output)"
    )
    parser.add_argument(
        "-l", "--language", help="Target programming language (auto-detected if not specified)"
    )
    parser.add_argument("-v", "--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument("--version", action="version", version="Article-to-Prototype v1.0.0")
    ...
    result = orchestrator.process(
        source=args.source, output_dir=args.output, language_hint=args.language
    )
