...
class SkillEvolutionAssistant(EvolvableSkill):
    ...
    def transform_all_skills(self) -> Dict[str, Any]:
        ...
        for skill_name in needs_evolution:
            ...
            results.append(
                {"skill": skill_name, "success": result["success"], "error": result.get("error")}
            )
    ...
    def _transform_main_file(self, skill_info: SkillInfo):
        ...
        if match:
            ...
            content = content.replace(
                f"class {class_name}{old_inheritance}", f"class {class_name}{new_inheritance}"
            )
...
"""
        ...
        for method_name in ["run", "execute", "process", "main"]:
            ...
            if re.search(pattern, content):
                content = re.sub(
                    f"def {method_name}\\(self", "def _execute_core(self", content, count=1
                )
                break
    ...
    def _add_evolution_config(self, skill_info: SkillInfo):
        ...
        # 复制模板
        template_path = (
            self.skills_root / "core" / "evolution" / "config" / "evolution_config_template.yaml"
        )
