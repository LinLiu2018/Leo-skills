...
""",
    ...
    def quick_capture(
        ...
        # 渲染内容
        note_content = self._render_template(
            "default", {"title": title, "content": content, "tags": all_tags, "links": []}
        )
    ...
    def update_moc(
        ...
        if moc_path.exists():
            ...
            # 创建新MOC
            content = self._render_template("moc", {"title": moc_name, "links": add_links or []})
        ...
        # 更新时间戳
        content = re.sub(
            r"\*最后更新: .*\*", f'*最后更新: {datetime.now().strftime("%Y-%m-%d %H:%M")}*', content
        )
    ...
    def add_links_to_note(self, note_path: str, links: List[str]) -> Dict[str, Any]:
        """
        向现有笔记添加链接。
        ...
        try:
            ...
            return {"success": True, "path": str(full_path), "added_links": links}
        except Exception as e:
            return {"success": False, "error": str(e)}

    def search_notes(
        self, query: str, folder: Optional[str] = None, limit: int = 20
    ) -> Dict[str, Any]:
        """
        搜索笔记。
    ...
    def get_recent_notes(self, limit: int = 10, folder: Optional[str] = None) -> Dict[str, Any]:
        ...
        for note in notes[:limit]:
            note["modified"] = datetime.fromtimestamp(note["modified"]).strftime("%Y-%m-%d %H:%M")
...
def main():
    ...
    parser.add_argument(
        "command", choices=["capture", "daily", "search", "recent", "help"], help="命令"
    )
