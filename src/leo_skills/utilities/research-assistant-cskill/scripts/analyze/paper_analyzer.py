...
class PaperAnalyzer:
    ...
    def _init_patterns(self):
        ...
        self.finding_patterns = [
            re.compile(
                r"(?i)(?:we\s+find|our\s+results?\s+(?:show|demonstrate|indicate|suggest)|found\s+that|results?\s+indicate)"
            ),
    ...
    def analyze(self, identifier: str, source: str = "auto") -> "Paper":
        """
    ...
    def _analyze_text(self, text: str, section: str = "general") -> Dict[str, Any]:
        """
    ...
    def _extract_contributions(self, text: str) -> List[str]:
        ...
        contribution_patterns = [
            re.compile(
                r"(?i)(?:our\s+(?:main\s+)?(?:contribution|contributions)\s+(?:is|are|involve|include)"
            ),
            re.compile(
                r"(?i)(?:this\s+(?:paper|study|work)\s+(?:makes?|provides?|offers?|presents?|introduces?|proposes?)\s+(?:a\s+)?(?:novel|new|significant|important|first)?\s*(?:contribution|approach|method|model|framework))"
            ),
    ...
    def _extract_limitations(self, text: str) -> List[str]:
        ...
        limitation_patterns = [
            re.compile(
                r"(?i)(?:(?:this|our)\s+(?:study|work|approach|method)\s+(?:is\s+)?(?:limited|has\s+limitations?|suffers?\s+from)\s*([^.?!\n]+[.?!\n])"
            ),
            re.compile(
                r"(?i)(?:limitat\w+\s+(?:include|are|of\s+this\s+study)\s*([^.?!\n]+[.?!\n])"
            ),
            re.compile(
                r"(?i)(?:future\s+work\s+should\s+(?:address|investigate|explore|consider)\s+([^.?!\n]+[.?!\n])"
            ),
        ]
    ...
    def _extract_references(self, text: str) -> List[Dict]:
        ...
        for pattern in ref_patterns:
            ...
            for match in matches:
                ...
                references.append({"number": ref_num, "text": ref_text_content.strip()[:300]})
    ...
    def compare_papers(self, papers: List["Paper"]) -> Dict[str, Any]:
        ...
        comparison["common_topics"] = sorted(all_topics.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
