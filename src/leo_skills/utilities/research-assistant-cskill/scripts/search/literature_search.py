    ...
    def _init_session(self):
        ...
        if self.session is None and requests:
            ...
            self.session.headers.update(
                {"User-Agent": "ResearchAssistant/1.0", "Accept": "application/json"}
            )
...
class ArxivSearch(BaseSearchEngine):
    ...
    @retry_on_failure(max_attempts=3)
    def search(
        self, query: str, max_results: int = 10, year_range: Optional[tuple] = None, **kwargs
    ) -> List[SearchResult]:
        """Search arXiv."""
        if not requests:
            logger.error("requests library not installed")
    ...
    def _parse_entry(self, entry, ns) -> SearchResult:
        ...
        if published_elem is not None and published_elem.text:
            try:
                year = datetime.fromisoformat(published_elem.text.replace("Z", "+00:00")).year
            except ValueError:
                pass
...
class OpenAlexSearch(BaseSearchEngine):
    ...
    def search(
        ...
        try:
            response = self.session.get(
                f"{self.BASE_URL}/works", params=params, headers=headers, timeout=30
            )
            response.raise_for_status()
            data = response.json()
...
class LiteratureSearch:
    ...
    def search(
        ...
        author is not None or venue is not None
    ...
    def find_citing_papers(self, paper_id: str, max_results: int = 20) -> List[SearchResult]:
        ...
        if "semantic_scholar" in self.engines:
            ...
            return engine.search(query=f"citations:{paper_id}", max_results=max_results)

        # Fallback: Try other methods
        logger.warning("Citation lookup not fully implemented for this database")
        return []

    def get_paper(self, identifier: str, source: str = "auto") -> Optional[SearchResult]:
        """
