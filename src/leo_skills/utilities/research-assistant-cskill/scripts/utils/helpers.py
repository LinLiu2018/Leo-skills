...
def setup_logging(
    level: int = logging.INFO, log_file: Optional[str] = None, format_str: Optional[str] = None
) -> logging.Logger:
    """
...
class CacheManager:
    ...
    def __init__(self, cache_dir: str = ".cache", enabled: bool = True, cache_days: int = 7):
        """
            ...
            while retries < max_retries:
                try:
                    ...
                    if any(
                        ...
                        logger.warning(
                            f"Rate limited, waiting {wait:.1f}s (attempt {retries + 1}/{max_retries})"
                        )
                        time.sleep(wait)
                        retries += 1
                    else:
...
def parse_author_string(author_str: str) -> Dict[str, str]:
    ...
    """
    result = {"first": "", "middle": "", "last": "", "original": author_str}
...
def format_author_name(first: str, last: str, middle: str = "", format: str = "last_first") -> str:
    """
