...
class AgentDiscovery:
    ...
    def discover_all(self) -> Dict[str, AgentMetadata]:
        ...
        Returns:
            发现的 Agent 元数据字典
        """
        print(f"[SEARCH] 扫描 Agent 目录: {self.agents_dir}")
        ...
        for item in self.agents_dir.iterdir():
            if item.is_dir() and not item.name.startswith(("_", ".")):
                ...
                if agent_meta:
                    ...
                    print(f"  [SUCCESS] 发现 Agent: {agent_meta.name} ({agent_meta.type})")

        print(f"[DATA] 共发现 {len(self.discovered_agents)} 个 Agent")
    ...
    def _load_from_manifest(self, manifest_path: Path, agent_dir: Path) -> Optional[AgentMetadata]:
        ...
        try:
            ...
            print(f"  [WARNING] 读取清单失败 {manifest_path}: {e}")
            return None
    ...
    def load_agent_class(self, agent_meta: AgentMetadata) -> Optional[Type]:
        ...
        if not module_file.exists():
            print(f"  [WARNING] 模块文件不存在: {module_file}")
            return None

        try:
            # 动态加载模块
            spec = importlib.util.spec_from_file_location(
                f"leo_subagents.agents.{agent_meta.name}", str(module_file)
            )
            ...
            if agent_class:
                ...
                print(f"  [WARNING] 未找到类 {agent_meta.class_name} in {module_file}")
                return None

        except Exception as e:
            print(f"  [WARNING] 加载 Agent 类失败 {agent_meta.name}: {e}")
            return None
