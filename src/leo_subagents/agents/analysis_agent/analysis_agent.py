...
class AnalysisAgent(BaseAgent):
    ...
    # 激活关键词
    ACTIVATION_KEYWORDS = ["分析", "统计", "趋势", "报告", "数据", "指标", "评估", "对比"]
    ...
    def _plan_analysis(self, task: str, analysis_type: str, **kwargs) -> List[Dict[str, Any]]:
        """
        规划分析步骤
        ...
        if analysis_type == "descriptive":
            # 描述性分析
            steps.append(
                {"step": 1, "name": "数据概览", "action": "summarize_data", "params": kwargs}
            )
            steps.append(
                {"step": 2, "name": "关键指标提取", "action": "extract_metrics", "params": kwargs}
            )

        elif analysis_type == "trend":
            # 趋势分析
            steps.append(
                {"step": 1, "name": "数据收集", "action": "collect_data", "params": kwargs}
            )
            steps.append(
                {"step": 2, "name": "趋势识别", "action": "identify_trends", "params": kwargs}
            )
            steps.append(
                {"step": 3, "name": "预测生成", "action": "generate_forecast", "params": kwargs}
            )

        elif analysis_type == "comparative":
            # 对比分析
            steps.append(
                {"step": 1, "name": "数据对比", "action": "compare_data", "params": kwargs}
            )
            steps.append(
                {"step": 2, "name": "差异分析", "action": "analyze_differences", "params": kwargs}
            )
    ...
    def _execute_analysis_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if action in action_mapping:
                ...
                if hasattr(execution_result, "result"):
                    result = (
                        ...
                        else {"data": execution_result.result}
                    )
                    ...
                    result = (
                        ...
                        else {"data": execution_result, "success": True}
                    )
            else:
            ...
            return {"step": step["step"], "name": step["name"], "success": False, "error": str(e)}

    def _generate_analysis_report(
        self, task: str, analysis_type: str, results: List[Dict[str, Any]], **kwargs
    ) -> Dict[str, Any]:
        """
        生成分析报告
    ...
    def _create_report_summary(
        self, task: str, analysis_type: str, results: List[Dict[str, Any]]
    ) -> str:
        """
        创建报告摘要
...
if __name__ == "__main__":
    # 创建配置
    config = AgentConfig(
        name="analysis-agent", type="analyzer", priority=3, skills=[], description="分析代理"
    )
