...
class CreativeAgent(BaseAgent):
    ...
    # 激活关键词
    ACTIVATION_KEYWORDS = ["创作", "撰写", "生成", "编写", "文案", "内容", "文章", "报告"]
    ...
    def _plan_creation(self, task: str, content_type: str, **kwargs) -> List[Dict[str, Any]]:
        """
        规划创作步骤
    ...
    def _execute_creation_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"step": step["step"], "name": step["name"], "success": True, "result": result}

        except Exception as e:
            return {"step": step["step"], "name": step["name"], "success": False, "error": str(e)}

    def _finalize_content(
        self, task: str, content_type: str, results: List[Dict[str, Any]], **kwargs
    ) -> Dict[str, Any]:
        """
        汇总和优化内容
        ...
        for result in successful_results:
            content_pieces.append(
                {"step_name": result["name"], "content": result.get("result", "")}
            )
