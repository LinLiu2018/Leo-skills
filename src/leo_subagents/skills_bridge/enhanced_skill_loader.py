...
class EnhancedSkillLoader:
    ...
    def __init__(self, skills_path: Optional[Path] = None, workflows_path: Optional[Path] = None):
        """初始化加载器"""
        ...
        self.stats = {"total_skills": 0, "total_workflows": 0, "by_category": {}, "scan_depth": 0}
    ...
    def discover_all(self) -> Dict[str, Any]:
        ...
        Returns:
            发现的统计信息
        """
        logger.info(f"扫描 Skills 目录: {self.skills_path}")
        ...
        logger.info(f"发现 {len(self.skills)} 个 Skills, {len(self.workflows)} 个 Workflows")

        return self.stats

    def _scan_skills_recursive(
        self, directory: Path, parent_category: str, depth: int, max_depth: int = 5
    ):
        """递归扫描目录发现 Skills"""
        if depth > max_depth:
            return
        ...
        for item in directory.iterdir():
            ...
            if self._is_skill_directory(item):
                ...
                if skill_info:
                    ...
                    logger.info(f"  {skill_info.name} ({cat})")
            else:
                # 递归扫描子目录
    ...
    def _is_skill_directory(self, directory: Path) -> bool:
        ...
        if (directory / "scripts").exists() and (
            ...
            or any(directory.glob("*.yaml"))
            or any(directory.glob("*.yml"))
        ):
            return True
    ...
    def _scan_workflows(self):
        """扫描 Workflows"""
        logger.info(f"扫描 Workflows 目录: {self.workflows_path}")
        ...
        for item in self.workflows_path.iterdir():
            if item.is_dir():
                ...
                if workflow_info:
                    ...
                    logger.info(f"  Workflow: {workflow_info.name}")
...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("Skills 汇总")
    logger.info("=" * 60)

    for cat, skills in sorted(loader.categories.items()):
        logger.info(f"{cat} ({len(skills)} 个):")
        for skill_name in skills[:10]:  # 只显示前10个
            skill = loader.skills[skill_name]
            logger.info(f"  • {skill_name}")
            if skill.description:
                logger.info(f"    {skill.description[:60]}...")
        if len(skills) > 10:
            logger.info(f"  ... 还有 {len(skills) - 10} 个")

    logger.info(f"Workflows ({len(loader.workflows)} 个):")
