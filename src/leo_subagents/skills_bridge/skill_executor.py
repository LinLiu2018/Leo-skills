...
class SkillExecutor:
    ...
    @track_time
    def execute(self, skill_name: str, action: str, **kwargs) -> ExecutionResult:
        """
        执行Skill操作
        ...
        if not adapter.validate_action(action):
            return ExecutionResult(
                skill_name=skill_name, action=action, success=False, error=f"无效的操作: {action}"
            )
    ...
    def _execute_action(self, adapter: SkillAdapter, action: str, **kwargs) -> Any:
        """
        实际执行操作
    ...
    def _load_skill_instance(self, adapter: SkillAdapter) -> Any:
        ...
        possible_files = [
            ...
            skill_path / "main.py",
            skill_path / "skill.py",
            skill_path / "scripts" / "main.py",  # Support scripts/ directory
        ]
        ...
        try:
            ...
            # 加载失败，记录错误但不中断
            logger.error(f"加载 skill 失败: {adapter.skill_name}, 错误: {e}")
    ...
    def execute_batch(self, tasks: list) -> list:
        ...
        for task in tasks:
            result = self.execute(
                skill_name=task["skill"], action=task["action"], **task.get("params", {})
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if total == 0:
            return {"total_executions": 0, "success_rate": 0.0, "by_skill": {}, "by_action": {}}
    ...
    def print_statistics(self):
        ...
        if stats["by_skill"]:
            logger.info("按Skill统计:")
            for skill, data in stats["by_skill"].items():
                logger.info(
                    f"  {skill}: {data['total']}次 (成功: {data['success']}, 失败: {data['failed']})"
                )

        if stats["by_action"]:
            logger.info("按操作统计:")
            for action, data in stats["by_action"].items():
                logger.info(f"  {action}: {data['total']}次 (成功: {data['success']})")
...
def execute_skill(skill_name: str, action: str, **kwargs) -> ExecutionResult:
    """
    便捷函数：执行Skill
...
if __name__ == "__main__":
    ...
    logger.info(f"执行结果: {result.to_dict()}")
    ...
    logger.info(f"批量执行结果: {len(results)}个任务")
