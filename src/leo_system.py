...
# 加载registry模块
registry = load_module_from_file(
    "leo_orchestrator.registry", str(current_path / "leo_orchestrator" / "registry.py")
)
api = load_module_from_file(
    "leo_orchestrator.api", str(current_path / "leo_orchestrator" / "api.py")
)
...
# 加载subagents模块
base_agent = load_module_from_file(
    "leo_subagents.agents.base_agent",
    str(current_path / "leo_subagents" / "agents" / "base_agent.py"),
)
AgentFactory = base_agent.AgentFactory
AgentConfig = base_agent.AgentConfig

skill_adapter = load_module_from_file(
    "leo_subagents.skills_bridge.skill_adapter",
    str(current_path / "leo_subagents" / "skills_bridge" / "skill_adapter.py"),
)
skill_loader = load_module_from_file(
    "leo_subagents.skills_bridge.skill_loader",
    str(current_path / "leo_subagents" / "skills_bridge" / "skill_loader.py"),
)
skill_executor_module = load_module_from_file(
    "leo_subagents.skills_bridge.skill_executor",
    str(current_path / "leo_subagents" / "skills_bridge" / "skill_executor.py"),
)
...
# 创建task_agent模块的依赖
task_agent_module = load_module_from_file(
    "leo_subagents.agents.task_agent",
    str(current_path / "leo_subagents" / "agents" / "task_agent.py"),
)
TaskAgent = task_agent_module.TaskAgent

# 加载新实现的3个Agent
try:
    research_agent_module = load_module_from_file(
        "leo_subagents.agents.research_agent",
        str(current_path / "leo_subagents" / "agents" / "research_agent" / "research_agent.py"),
    )
    ResearchAgent = research_agent_module.ResearchAgent
    # 手动注册到AgentFactory
    AgentFactory.register_agent_class("researcher", ResearchAgent)
    print("[SUCCESS] 注册ResearchAgent到AgentFactory")
except Exception as e:
    print(f"[WARNING]  加载ResearchAgent失败: {e}")

try:
    analysis_agent_module = load_module_from_file(
        "leo_subagents.agents.analysis_agent",
        str(current_path / "leo_subagents" / "agents" / "analysis_agent" / "analysis_agent.py"),
    )
    AnalysisAgent = analysis_agent_module.AnalysisAgent
    # 手动注册到AgentFactory
    AgentFactory.register_agent_class("analyzer", AnalysisAgent)
    print("[SUCCESS] 注册AnalysisAgent到AgentFactory")
except Exception as e:
    print(f"[WARNING]  加载AnalysisAgent失败: {e}")

try:
    creative_agent_module = load_module_from_file(
        "leo_subagents.agents.creative_agent",
        str(current_path / "leo_subagents" / "agents" / "creative_agent" / "creative_agent.py"),
    )
    CreativeAgent = creative_agent_module.CreativeAgent
    # 手动注册到AgentFactory
    AgentFactory.register_agent_class("creator", CreativeAgent)
    print("[SUCCESS] 注册CreativeAgent到AgentFactory")
except Exception as e:
    print(f"[WARNING]  加载CreativeAgent失败: {e}")

try:
    realestate_agent_module = load_module_from_file(
        "leo_subagents.agents.realestate_agent",
        str(current_path / "leo_subagents" / "agents" / "realestate_agent" / "realestate_agent.py"),
    )
    RealEstateAgent = realestate_agent_module.RealEstateAgent
    # 手动注册到AgentFactory
    AgentFactory.register_agent_class("realestate", RealEstateAgent)
    print("[SUCCESS] 注册RealEstateAgent到AgentFactory")
except Exception as e:
    print(f"[WARNING]  加载RealEstateAgent失败: {e}")
...
class LeoSystem:
    ...
    def _create_agents(self):
        ...
        for agent_name, agent_reg in registry.agents.items():
            ...
            try:
                ...
                print(f"  [SUCCESS] 创建Agent: {agent_name} ({agent_reg.type})")
            except Exception as e:
                print(f"  [WARNING]  创建Agent失败 {agent_name}: {e}")
    ...
    def execute_task(self, task: str, agent_name: str = None, **kwargs) -> Dict[str, Any]:
        ...
        if agent_name:
            if agent_name not in self.agents:
                return {"success": False, "error": f"Agent不存在: {agent_name}"}
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agents": {"total": len(self.agents), "names": list(self.agents.keys())},
    ...
    def print_status(self):
        ...
        # Skills统计
        print(f"\n[TARGET] Skills: {len(self.skill_loader.skills)} 个")
...
def main():
    ...
    if args.status:
        ...
        if ":" not in args.call:
            print("[ERROR] 格式错误，应为 skill_name:action")
            return
